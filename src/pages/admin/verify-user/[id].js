...
const Verify = ({ data }) => {
  ...
  const { name, email, phone, organisation, qrCode, active } = data || "";

  const printDocument = () => {
    var prtContent = document.getElementById("user-details-id");
    ...
    WinPrint.document.write('<link rel="stylesheet" href="./globals.css" />');
  ...
  return (
    <>
      ...
      >
        <input
          type="submit"
          style={{ width: "200px", backgroundColor: "#2c8c45", border: "none" }}
...
export async function getServerSideProps(context) {
  try {
    ...
    const response = await axiosMain.get(`/admin/verifyUser/?id=${id}`);
    ...
    return {
      redirect: {
        permanent: false,
        destination: "/auth/user-management",
      },
      props: {},
    };
  }
